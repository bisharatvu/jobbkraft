class LinkedinController < ApplicationController
  ...
  def auth
    init_client
    puts "http://#{request.host_with_port}/linkedin/callback"
  ...
  def callback
    ...
    a = c.profile(fields: ['first_name','last_name', 'picture-urls::(original)', 'phone-numbers', 'location'])
    @user = current_user
    @user.user_first_name = a.first_name
    @user.user_last_name = a.last_name
    @user.remote_image_url_url = a.pictureUrls.all.first
    @user.name = a.first_name +' '+ a.last_name
    @user.user_cell_phone = a.phoneNumbers
    @user.save!
=begin

    puts "Profile Information#{a.pictureUrls.all.first}"
    a.inspect
=end
...
=end
    ...
    session[:atoken] = nil
    session[:asecret] = nil
    redirect_to profile_path(:imported_from_linkedin=>"success")
  end
